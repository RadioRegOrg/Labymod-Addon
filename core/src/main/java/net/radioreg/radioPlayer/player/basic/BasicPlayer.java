 ...
 *----------------------------------------------------------------------
 */
package net.radioreg.radioPlayer.player.basic;
...
public class BasicPlayer implements BasicController, Runnable {
  ...
  private static Logging log = Main.get().logger();
      ...
      while ((nBytesRead != -1) && (m_status != STOPPED) && (m_status != SEEKING) && (m_status
        ...
        if (m_status == PLAYING) {
          ...
          try {
            ...
            if (nBytesRead >= 0) {
              ...
              if (m_line.available() >= m_line.getBufferSize()) {
                log.info("Underrun : " + m_line.available() + "/" + m_line.getBufferSize());
              }
          ...
          try {
            ...
            log.warn("Thread cannot sleep(1000)", e);
          }
        }
      }
